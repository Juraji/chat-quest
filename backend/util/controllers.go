...
func RespondList[T any](cq *cq.ChatQuestContext, c *gin.Context, records []T, err error) {
	if err != nil {
...
func RespondSingle[T any](cq *cq.ChatQuestContext, c *gin.Context, entity *T, err error) {
	if err != nil {
		RespondInternalError(cq, c, err)
	} else if entity == nil {
...
func RespondEmpty(cq *cq.ChatQuestContext, c *gin.Context, err error) {
	if err != nil {
		RespondInternalError(cq, c, err)
	} else {
...
func RespondDeleted(cq *cq.ChatQuestContext, c *gin.Context, err error) {
	if err != nil {
...
func RespondInternalError(cq *cq.ChatQuestContext, c *gin.Context, err error) {
	c.JSON(http.StatusInternalServerError, gin.H{"error": "Internal Server Error"})
	cq.Logger().Error("Error during API request", zap.String("uri", c.Request.RequestURI), zap.Error(err))
}

func RespondBadRequest(cq *cq.ChatQuestContext, c *gin.Context, message string) {
	c.JSON(http.StatusBadRequest, gin.H{"error": message})
	cq.Logger().Warn("Bad API Request", zap.String("uri", c.Request.RequestURI), zap.String("message", message))
}

func RespondNotAcceptable(cq *cq.ChatQuestContext, c *gin.Context, message string, err error) {
