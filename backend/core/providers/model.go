...
import (
	...
	util2 "juraji.nl/chat-quest/core/util"
	"strings"
)
...
func AllConnectionProfiles(cq *core.ChatQuestContext) ([]*ConnectionProfile, error) {
	query := "SELECT * FROM connection_profiles"
	return database.QueryForList(cq.DB(), query, nil, connectionProfileScanner)
}

func ConnectionProfileById(cq *core.ChatQuestContext, id int) (*ConnectionProfile, error) {
	query := "SELECT * FROM connection_profiles WHERE id = ?"
	args := []any{id}
	return database.QueryForRecord(cq.DB(), query, args, connectionProfileScanner)
}

func CreateConnectionProfile(cq *core.ChatQuestContext, profile *ConnectionProfile, llmModels []*LlmModel) error {
	tx, err := cq.DB().Begin()
	if err != nil {
	...
	util2.EmitAll(cq, LlmModelCreatedSignal, llmModels)
	return nil
}

func UpdateConnectionProfile(cq *core.ChatQuestContext, id int, profile *ConnectionProfile) error {
...
func DeleteConnectionProfileById(cq *core.ChatQuestContext, id int) error {
...
func LlmModelsByConnectionProfileId(cq *core.ChatQuestContext, profileId int) ([]*LlmModel, error) {
	query := "SELECT * FROM llm_models WHERE connection_profile_id = ?"
	args := []any{profileId}
	return database.QueryForList(cq.DB(), query, args, llmModelScanner)
}

func CreateLlmModel(cq *core.ChatQuestContext, profileId int, llmModel *LlmModel) error {
...
func UpdateLlmModel(cq *core.ChatQuestContext, id int, llmModel *LlmModel) error {
...
func DeleteLlmModelById(cq *core.ChatQuestContext, id int) error {
...
func MergeLlmModels(cq *core.ChatQuestContext, profileId int, newModels []*LlmModel) error {
	tx, err := cq.DB().Begin()
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
	}
	...
	// New model id set
	newModelIdSet := util2.NewSetFrom(newModels, func(t *LlmModel) string {
		return t.ModelId
	})
	...
	existingModelIdSet := util2.NewSetFrom(existingModels, func(t *LlmModel) string {
		return t.ModelId
	})
	...
	util2.EmitAll(cq, LlmModelCreatedSignal, createdModels)
	util2.EmitAll(cq, LlmModelDeletedSignal, deletedModelIds)
	return nil
}

func GetAllLlmModelViews(cq *core.ChatQuestContext) ([]*LlmModelView, error) {
